// 유저에게 비밀번호 설정을 물어보고, 비밀번호가it,IT를 포함하고 비밀번호 설정 완료! 아니면 비밀번호 설정 오류!

// const a = "it" || "IT"; //it
// const password = prompt("비밀번호 설정");
// const isPass = password.includes("it") || password.includes("IT");
// const msg = isPass ? "비밀번호 설정완료!" : "비밀번호 설정 불가!";
// alert(msg);

// //1. 유저에게 좋아하는 음식을 입력 받고 그 음식을 5번 출력하는 프로그램
// const favorit = prompt("좋아하는 음식은?");
// const fiveFavorit = favorit.repeat(5);
// alert(fiveFavorit);
// //2. 유저에게 좋아하는 음식과 횟수를 입력 받고 그 음식을 횟수번 만큼 출력하는 프로그램
// //  ex)떡볶이 5 -> 떡볶이떡볶이떡볶이떡볶이떡볶이

// const foodCount = prompt("좋아하는 음식과 횟수는?");
...
// -2. 시작이 it,IT로 시작 안하면 비밀번호 시작은 it,IT 시작해야함
// -3. 특수문자 &#@!중에 하나라도 없으면 비밀번호에 반드시 특수문자 넣어야하며
// 위에 조건이 모두 통과되면 비밀번호 설정완료!

// const word = prompt("영단어를 입력해주세요");
// const upperword = word.toUpperCase() ? word.toUpperCase() : word.toLowerCase()

// alert(updderword)

// const a = "it" || "IT"
// const password =prompt("비밀번호 설정")
// const isPass = password.includes("it") || password.includes("IT") || password.includes("&#@!");
// const leng = ispass.length Number(prompt(8 < 20));
// const msg = ispass ? ""
